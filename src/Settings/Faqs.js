...
export default class Faqs extends Component {
    ...
    faq_data = async () => {
        let url = config.baseURL + "get_all_faq.php?user_id=" + 0;
        console.log(url)
        apifuntion.getApi(url).then((obj) => {
            if (obj.success == "true") {
                consolepro.consolelog('objjjjjj', obj.faq_arr);
                this.setState({ myfaq_arr: obj.faq_arr });
            } else {
                                    ...
                                    <View style={styles.Container}>
                                        <View style={{ width: '100%', paddingBottom: mobileW * 3 / 100, marginTop: mobileW * 8 / 100, paddingHorizontal: mobileW * 6 / 100, flexDirection: 'row', alignSelf: 'center' }}>
                                            ...
                                            <View style={{ width: '85%', marginLeft: mobileW * -2 / 100 }}>
                                                <Text style={{ fontFamily: Font.semibold_font, fontSize: mobileW * 4 / 100, Colors: Colors.black_color }}>{item.question}</Text>
                                            </View>
                                        ...
                                        {item.status == true && <View style={{ width: '85%', alignSelf: 'center', paddingHorizontal: mobileW * 1.5 / 100 }}>
                                            <Text style={{ fontSize: mobileW * 3.5 / 100, fontFamily: Font.semibold_font, color: Colors.textcolor }}>{item.answer}</Text>
                                        </View>}
                                    </View>
                                )
