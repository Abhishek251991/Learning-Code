...
export default function OtpVerification() {
  ...
  const verify_click = async () => {
    ...
    // let otpstatus = otp1 + otp2 + otp3 + otp4;

    // consolepro.consolelog('otpstatus', otpstatus)

    const reg = /^\d+$/;
    if (reg.test(otp1) !== true) {
        ...
        <TouchableOpacity activeOpacity={1} style={{ flex: 1 }} onPress={() => { Keyboard.dismiss() }}>
          ...
          <View style={{ width: '95%', alignSelf: 'center' }}>
            <View style={{ width: '100%', alignItems: 'center', alignSelf: 'center', marginTop: mobileW * -10 / 100, marginBottom: mobileW * -4 / 100 }}>
              <Image source={require('../icons/logo.png')} resizeMode='contain' style={{ width: mobileW * 33 / 100, height: mobileW * 33 / 100 }} />
            </View>
