...
messagecountforfooter=async()=>{
 ...
 getMyInboxAllData=async()=>{
	...
	console.log('getMyInboxAllData firebase');
	...
	console.log('FirebaseInboxJson himanhu',FirebaseInboxJson);
	...
	var queryUpdate = firebase.database().ref('users/'+id+'/myInbox/');
		...
		console.log('inboxKeyName',inboxKeyName);
			...
			var job_id=data.val().job_id
			var service_name=data.val().service_name
			var job_number=data.val().job_number
			...
			for (var i = 0; i < FirebaseInboxJson.length; i++) {
				if (FirebaseInboxJson[i].job_id == job_id) {
				  ...
				  FirebaseInboxJson[i].job_number = job_number;
				  FirebaseInboxJson[i].service_name = service_name;
	...
	var queryGroup = queryGroup.on('child_removed',(data)=> {
	
		consolepro.consolelog('inbox update removed',data.toJSON());	
		consolepro.consolelog('inbox update removed job_id',data.val().job_id);	
		
		consolepro.consolelog('FirebaseGroupJson check himanshu',FirebaseGroupJson);
		var job_id=data.val().job_id;
		var inboxKeyName = data.key.charAt(0);

		for (var i = 0; i < FirebaseGroupJson.length; i++) {
			if (FirebaseGroupJson[i].job_id == job_id) {
				FirebaseGroupJson.splice(i,1);
			
					$('#job_id_'+job_id).hide();
					$('#job_id_'+job_id).remove();
