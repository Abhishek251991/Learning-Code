...
export default class Big_Project extends Component {
  constructor(props) {
    super(props)
    this.state = {
      openJobs: true,
  ...
  render() {
    return (
      <Container backgroundColor={Colors.whiteColor}>
        <Header
          goBack={() => { this.props.navigation.navigate('My_Jobs') }}
                  ...
                  activeOpacity={0.8}
                    ...
                    <View style={{ width: '90%', justifyContent: 'space-between', alignSelf: 'center', flexDirection: 'row' }}>
                      <View style={{ width: '68%' }}>
                        ...
                        {/* ----------------View Quotation Buton Start-------------------- */}
                      {item.job_status == 0 && <TouchableOpacity
                       activeOpacity={1} 
                       disabled={true} 
                       onPress={() => { this.props.navigation.navigate('Big_Project_Openjob_Details', { arr: item }) }} style={{ width: '32%',backgroundColor: '#1AA4D0', borderRadius: mobileW * 1.5 / 100, paddingVertical: mobileW * 1 / 100}}>
                        <Text style={{ textAlign: 'center', fontSize: mobileW * 3.5 / 100, color: Colors.whiteColor, fontFamily: Font.regular_font }}>{Lang_chg.quotationbtn[config.language]}</Text>
                      </TouchableOpacity>}
                      {/* ----------------View Quotation Buton End-------------------- */}
                      {item.job_status != 0 && <View
                        style={{
                          width: '30%',
                          flexDirection: 'row',
                          alignItems: 'center',
                        ...
                        <View
                          ...
                          <Text style={{
                            width: '100%', textAlign: 'left', fontFamily: Font.regular_font, color: (item.user_job_status[config.language] == 'Pending') || (item.user_job_status[config.language] == 'Verserende') ? Colors.red_Color
